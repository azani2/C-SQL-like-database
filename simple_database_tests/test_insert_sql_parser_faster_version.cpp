...
TEST_CASE("rows_from_line_faster correctly parses simple table row") {
	...
	insert_sql_parser::rows_from_line_faster(s, t, actual);
...
TEST_CASE("rows_from_line_faster correctly parses simple table rows") {
...
TEST_CASE("rows_from_line_faster correctly parses table row with index") {
...
TEST_CASE("rows_from_line_faster correctly parses rows with different types") {
...
TEST_CASE("rows_from_line_faster correctly parses rows with default values and missing columns") {
...
TEST_CASE("rows_from_line_faster correctly parses rows with index column in the middle") {
...
TEST_CASE("rows_from_line_faster correctly parses rows with specified values for columns with default values") {
...
TEST_CASE("rows_from_line_faster correctly parses rows with specified and unspecified defaults") {
...
TEST_CASE("rows_from_line_faster correctly parses rows with specified and unspecified defaults of different types") {
...
TEST_CASE("rows_from_line_faster correctly parses datetime with indexed column") {
...
TEST_CASE("rows_from_line_faster correctly parses escaped ',' and ')' in string columns") {
...
TEST_CASE("rows_from_line_faster correctly parses quotes inside string columns") {
	std::string s = R"(INTO Sample {(1, "He said "Hello!"", 5)})";
	table t;
	t.info = table_info{ "Sample",3,0,false,0,0 };
	t.cols_info = {
		col_info{"ID", INT, false, 0, false, 0},
...
TEST_CASE("rows_from_line_faster correctly parses rows with negative integer and double values") {
...
TEST_CASE("rows_from_line_faster correctly applies default value for middle column") {
...
TEST_CASE("rows_from_line_faster correctly parses rows with index and default values") {
...
TEST_CASE("rows_from_line_faster correctly parses many rows") {
...
TEST_CASE("rows_from_line_faster correctly parses rows with date datetime and double") {
...
TEST_CASE("rows_from_line_faster correctly parses rows with date datetime and double specified and unspecified defaults") {
	std::string s =
		"INTO Sample {"
...
void test_row_parser_for_n_rows(const size_t& ROWS) {
	table t;
	t.info = { "Table",5,0,true,0,1000 };
	t.cols_info = {
		col_info{"ID",INT,false,0,true,0 },
	...
	std::cout << "\nTime for " << ROWS << " rows inserted (faster version): " << std::endl;
	...
	dur = end - start;
	std::cout << "rows_from_line_faster: " << std::to_string(dur.count()) << std::endl;
	
	compare_rows(expected, actual);
}
/*
TEST_CASE("rows_from_line_faster correctly parses 10 000 random rows") {
	test_row_parser_for_n_rows(10'000);
}

TEST_CASE("rows_from_line_faster correctly parses 100 000 random rows") {
	test_row_parser_for_n_rows(100'000);
}/* Takes 1.5min
TEST_CASE("rows_from_line_faster correctly parses a million random rows") {
	test_row_parser_for_n_rows(1'000'000);
}
*/

TEST_CASE("rows_from_line_faster throws when column record count doesn't match the table's expected column count") {
	...
	std::vector<row> actual;
	REQUIRE_THROWS_WITH(insert_sql_parser::rows_from_line_faster(s, t, actual), "Row #1 has invalid number of columns: 2, expected: 3.");
}


TEST_CASE("rows_from_line_faster throws if \" {\" is missing") {
	std::string s = "INTO Sample (1,2,3)}";
	table t = simple_table();
	std::vector<row> v;


	REQUIRE_THROWS_WITH(insert_sql_parser::rows_from_line_faster(s, t, v), "Invalid insert query: Table name cannot be empty and must be followed by \" {rows to be inserted}\".");
}
...
TEST_CASE("rows_from_line_faster throws if rows are empty \"{}\"") {
	std::string s = "INTO Sample {}";
	table t = simple_table();
	std::vector<row> v;


	REQUIRE_THROWS_WITH(insert_sql_parser::rows_from_line_faster(s, t, v), "Invalid insert query: Rows schema cannot be empty.");
}

TEST_CASE("rows_from_line_faster throws if row closing bracket is missing") {
	std::string s = "INTO Sample {(1,2,3}";
	table t = simple_table();
	std::vector<row> v;


	REQUIRE_THROWS_WITH(insert_sql_parser::rows_from_line_faster(s, t, v), "Invalid insert query: Rows must be described as \"(value, ..., value)\".");
}

TEST_CASE("rows_from_line_faster throws if there is text after \"}\"") {
	std::string s = "INTO Sample {(1,2,3)} as 2, 3aff afe";
	table t = simple_table();
	std::vector<row> v;


	REQUIRE_THROWS_WITH(insert_sql_parser::rows_from_line_faster(s, t, v), "Invalid insert query: There is unparsable text after end of rows.");
}

TEST_CASE("rows_from_line_faster throws if the line is random text") {
	std::string s = "gd, af hs\\ hzr45haf 1752526 gsd INTO ";
	table t = simple_table();
	std::vector<row> v;

	REQUIRE_THROWS_AS(insert_sql_parser::rows_from_line_faster(s, t, v), std::invalid_argument);
